...
mpool *mpool_init(int min2, int max2)
{
	...
	printf("mpool_init cnt=%d, palen=%d, min_pool=%d, max_pool=%d\n", cnt, palen, mp->min_pool, mp->max_pool);
...
void *mpool_alloc(mpool *mp, int sz)
{
    ...
    assert(szceil > 0);
//	printf("sz=%d, mpool_alloc i=%d, addr of hs[i]=%p\n", sz, i, mp->hs[i]);
